...
/**
 * Created by andycugb on 15-1-13.
 */
public class ExtendedShardedJedisPipeline extends Queable {
    ...
    /**
     * 默认的BuilderFactory为ZSET_BINARY，若按此方式使用会报错：java.io.StreamCorruptedException: invalid stream
     * header: EFBFBDEF<br>
    ...
    /**
     * add by xjyin 默认的BuilderFactory为ZSET_BINARY，若按此方式使用会报错：java.io.StreamCorruptedException:
    ...
    public Response<Set<Tuple>> zrevrangeByScoreWithScores(String key, double min, double max,
            int offset, int count) {
        Client c = getClient(key);
